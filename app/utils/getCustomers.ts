...
export const getCustomers = async (shopId: string) => {
  ...
  return customers.map((customer) => {
    ...
    return {
      ...
      name: customer.displayName || (customer.firstName && customer.lastName ? `${customer.firstName} ${customer.lastName}` : `Customer ${customer.shopifyCustomerId.slice(-4)}`),
      email: customer.email || `customer_${customer.shopifyCustomerId.slice(-6)}@example.com`,
